...
class Question1SearchFragment : Fragment() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        actionBar?.setTitle(R.string.search_view_question1)
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        val v = inflater.inflate(R.layout.fragment_search_question1, container, false)
        return setup(v)
    }
    ...
    private fun showQiitaListFragment(qiitaArticleResponse: ArrayList<QiitaArticleResponse>) {
        fragmentManager?.let {
            ...
            fragmentTransaction.replace(R.id.search_layout_frame, Question1QiitaListFragment.newInstance(qiitaArticleResponse))
                    .addToBackStack(null)
                    .commit()
        }
    }
